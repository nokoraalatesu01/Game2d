...
public class TheEndScreen implements Screen {
    ...
    private void drawEndOverlay(java.awt.Graphics2D g) {
        ...
        String playerLine = "Player: " + game.getCurrentPlayerName();
        String timeLine = "Finish time: " + game.getFormattedRunTime();
        ...
        g.setColor(new Color(240, 250, 225));
        FontMetrics infoMetrics = g.getFontMetrics();
        g.drawString(playerLine, (Core.VIRTUAL_WIDTH - infoMetrics.stringWidth(playerLine)) / 2, subtitleY + 34);
        g.drawString(timeLine, (Core.VIRTUAL_WIDTH - infoMetrics.stringWidth(timeLine)) / 2, subtitleY + 58);
        drawLeaderboardPreview(g, subtitleY + 92);
    ...
    private void drawLeaderboardPreview(java.awt.Graphics2D g, int startY) {
        List<LeaderboardEntry> entries = game.getLeaderboardEntries();
        g.setFont(new Font("Serif", Font.BOLD, 16));
        g.setColor(new Color(240, 250, 225));
        String title = "Top 5 Fastest";
        FontMetrics titleMetrics = g.getFontMetrics();
        g.drawString(title, (Core.VIRTUAL_WIDTH - titleMetrics.stringWidth(title)) / 2, startY);
        g.setFont(new Font("Serif", Font.PLAIN, 15));
        FontMetrics rowMetrics = g.getFontMetrics();

        if (entries.isEmpty()) {
            String status = game.getLeaderboardStatus();
            g.drawString(status, (Core.VIRTUAL_WIDTH - rowMetrics.stringWidth(status)) / 2, startY + 24);
            return;
        }
        ...
        for (LeaderboardEntry entry : entries) {
            String line = rank + ". " + entry.getPlayerName() + " - " + game.formatTime(entry.getElapsedMillis() / 1000f);
            g.drawString(line, (Core.VIRTUAL_WIDTH - rowMetrics.stringWidth(line)) / 2, y);
            y += 20;
            rank++;
            if (rank > 5) {
                break;
    ...
    private void updateEndOverlayInput(InputState input) {
        ...
        if (input.isKeyJustPressed(KeyEvent.VK_ENTER)) {
            ...
            if (selectedEndOption == EndOption.REPLAY) {
                game.startGameForPlayer(game.getCurrentPlayerName());
            } else {
    ...
    private void drawRunTimer(java.awt.Graphics2D g) {
        String timerText = "Time " + game.getFormattedRunTime();
        g.setFont(new Font("Serif", Font.BOLD, 18));
        FontMetrics metrics = g.getFontMetrics();
        int width = metrics.stringWidth(timerText) + 20;
        int x = Core.VIRTUAL_WIDTH - width - 16;
        int y = 16;
        g.setColor(new Color(0, 0, 0, 160));
        g.fillRoundRect(x, y, width, 32, 10, 10);
        g.setColor(new Color(238, 245, 220));
        g.drawRoundRect(x, y, width, 32, 10, 10);
        g.drawString(timerText, x + 10, y + 21);
    }
